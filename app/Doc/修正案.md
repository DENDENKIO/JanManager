...
| ファイル | 修正内容 | 状態 |
|---------|---------|------|
| `ui/scan/ScanViewModel.kt` | LinkageSlot enumをPIECE/PACK/CASEに変更、包装紐づけState追加、executePackageLinkage()実装 | ✅ 済 |
| `ui/scan/components/LinkageMode.kt` | スロットを単品・パック・ケースに全面変更、PackageSlotCardを刷新 | ✅ 済 |
| `ui/product/ProductDetailScreen.kt` | 包装単位の3種別選択、削除ボタン追加、OutlinedTextField統一、ステータスチップ、戻るボタン追加 | ✅ 済 |
| `ui/product/ProductDetailViewModel.kt` | deletePackageUnit()メソッド追加 | ✅ 済 |
| `data/repository/ProductRepository.kt` | deletePackageUnit()メソッド追加 | ✅ 済 |
| `data/local/entity/Enums.kt` | PackageType に displayName プロパティを追加 | ✅ 済 |
...
| 優先度 | 機能 | 理由 | 状態 |
|--------|------|------|------|
| 🔴 最高 | 包装単位の削除機能 | 1度登録すると消せない現状は運用不可 | ✅ 済 |
| 🔴 最高 | 包装単位 3種別（単品・パック・ケース）選択 | 仕様書通りの機能が未実装 | ✅ 済 |
| 🔴 最高 | LinkageModeのスロット修正 | リニューアル紐づけと包装紐づけが混在して動作しない | ✅ 済 |
| 🟡 高 | 入数入力フィールドの追加 | ケース・パックの入数が常にデフォルト値になる | ✅ 済 |
| 🟡 高 | PackageType displayName 追加 | UIの日本語化で可読性向上 | ✅ 済 |
| 🟢 中 | OutlinedTextField統一 | Material3の推奨スタイルへ統一 | ✅ 済 |
| 🟢 中 | ステータスチップ化 | 英語名の表示を日本語に改善 | ✅ 済 |
| 🟢 中 | TopAppBar 戻るボタン追加 | UX改善 | ✅ 済 |
| 🟢 低 | InfoRow 2列レイアウト | 商品情報の可読性向上 | ✅ 済 |
